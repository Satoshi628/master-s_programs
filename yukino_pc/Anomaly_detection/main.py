...
from models.sampler import KNN_sampler, Balance_sampler, Farthest_Point_Sampler
...
############## evalation関数 ##############
def evalation(cfg, train_dataloader_dict, test_dataloader_dict, device):
    ...
    sampler = Farthest_Point_Sampler(0.1, device)
    model.load(
...
def main(cfg):
    ...
    # evalation
    start = time.time()
    matrics = evalation(cfg, train_loader, val_loader, device)
    end = time.time()

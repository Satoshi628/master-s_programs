...
class Contrastive_Loss_moepy5(torch.nn.Module):
    ...
    def class_balance_choice(self, feature, targets, classes=0):
        ...
        PD = one_hot_changer(targets, self.n_class, dim=1)
        class_rate = PD.sum(dim=(-2, -1))
        class_rate = 1 / torch.clamp(class_rate, min=1e-7) / self.n_class
        PD = PD * class_rate[:, :, None, None]
        PD = PD.sum(dim=1)
        PD = PD / torch.clamp(PD.sum(dim=(-2, -1), keepdim=True), min=1e-7)  #正規化
        PD = PD.flatten(-2)  #マップ1枚からサンプリングするためフラットにする
...
class Contrastive_Loss_moepy_auto(torch.nn.Module):
    def __init__(self, n_class, choice_type="uniform", loss_type="entropy", edge_range=3, n=64, weight=None):
        super().__init__()
        self.n_class = n_class
        self.n = torch.Size([n])
        self.kappa = nn.Parameter(torch.Tensor(np.random.normal(size=(1))))
        self.avgPool = nn.AvgPool2d(2 * edge_range + 1, stride=1, padding=edge_range)
        self.CE = nn.CrossEntropyLoss(weight=weight)
    ...
    def Loss_Cos_11(self, anchor_targets, child_targets, anchor_feature, child_feature):
        ...
        exp_2k =math.exp(-2*self.kappa)
        sqrt_assurance = torch.clamp(2. - 2. * cos_sim, min=1e-7)
        cos_sim = 2. * (torch.exp(-(sqrt_assurance) ** (1 / (2 * self.kappa))) - exp_2k).div(1 - exp_2k) - 1.
        ...
        # torch.clampはnan対策。まるめ誤差によりcos_simは1を超える場合あり
        sqrt_assurance = torch.clamp(2. - 2. * anchor_cos_sim, min=1e-7)
        anchor_cos_sim = 2. * (torch.exp(-(sqrt_assurance) ** (1 / (2 * self.kappa))) - exp_2k).div(1 - exp_2k) - 1.
...
class CLLE_Loss(torch.nn.Module):
    ...
    def Within_image_loss(self, feature, JIT_feature, targets):
        ...
        for class_num in range(self.n_class):
            ...
            Loss = Loss - torch.log(cos_sim[match_flag]).sum() / match_flag.sum()
    ...
    def Cross_image_loss(self, feature, JIT_feature, targets):
        ...
        #1つずらす
        JIT_feature = torch.roll(JIT_feature, 1, dim=0)
        roll_targets = torch.roll(targets, 1, dim=0)
        # [b,N,N]
        ...
        for class_num in range(self.n_class):
            ...
            Loss = Loss - torch.log(class_cos[match_flag]).sum() / class_match_flag.sum()
    ...
    def forward(self, output, JIT_output, feature, JIT_feature, targets):  # output 出力(64次元)  targets ラベル
        ...
        Loss = 0
        Loss = Loss + self.CE(output, targets)
        Loss = Loss + self.CE(JIT_output, targets)

        output, JIT_output, feature, JIT_feature, targets
        feature = F.normalize(feature, dim=1).flatten(-2)
        JIT_feature = F.normalize(JIT_feature, dim=1).flatten(-2)
        targets = targets.flatten(-2)

        Loss = Loss + self.Within_image_loss(feature, JIT_feature, targets)
        Loss = Loss + self.Cross_image_loss(feature, JIT_feature, targets)
...
# Contrastive Learning for Label Efficient Semantic Segmentation
class CPC_Loss(torch.nn.Module):
    def __init__(self, n_class, weight=None):
        super().__init__()
        self.n_class = n_class
        self.CE = nn.CrossEntropyLoss(weight=weight)

    def CPC_loss(self, pred_feature, encd_feature):
        
        # [b,N]
        cos_sim = (pred_feature * pred_feature).sum(dim=1)
        cos_sim = torch.exp(cos_sim / 0.07)
        
        # [b,N,N]
        negative_cos_sim = (pred_feature[:, :, :, None] * pred_feature[:, :, None]).sum(dim=1)
        idx = torch.randperm(negative_cos_sim.shape[-1])[:100]
        negative_cos_sim = negative_cos_sim[:, :, idx]
        negative_cos_sim = torch.exp(negative_cos_sim / 0.07)

        # softmax
        cos_sim = cos_sim / negative_cos_sim.sum(dim=-1)

        Loss = - torch.log(cos_sim).mean()

        return Loss

    def forward(self, output, feature, targets):  # output 出力(64次元)  targets ラベル
        #class_feature => [tensor[b,c,H,W], ..., tensor[b,c,H,W]] 
        #output.size() => [b,c,h,w] [4, 64, 256, 256]
        #targets.size() => [b,h,w] [4, 256, 256]
        #print("outputkata", type(output))  <class 'torch.Tensor'>
        ...
        pred_feature = F.normalize(feature[0], dim=1).flatten(-2)
        encd_feature = F.normalize(feature[1], dim=1).flatten(-2)

        Loss = Loss + self.CPC_loss(pred_feature, encd_feature)

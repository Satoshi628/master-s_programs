...
from utils.dataset import Covid19_Loader, Drosophila_Loader, Drive_Loader
...
def dataload_DRIVE():
    ### data augmentation + preprocceing ###
    train_transform = ut.Compose([ut.RandomCrop(size=(256, 256)),  # ランダムクロップ(size=512x1024)
                                  # ランダムに左右反転(p=probability)
                                  ut.RandomHorizontalFlip(p=0.5),
                                  ut.RandomVerticalFlip(p=0.5),
                                  ut.ToTensor(),  # 0~255 ==> 0~1 + Tensor型に変換
    ...
    train_dataset = Drive_Loader(root_dir="/mnt/kamiya/dataset/DRIVE_retina", split=train_split, transform=train_transform)
    val_dataset = Drive_Loader(root_dir="/mnt/kamiya/dataset/DRIVE_retina", split=test_split, transform=val_transform)
...
if __name__ == '__main__':
    # dataset判定
    #dataset = ["Covid19", "Drosophila","DRIVE"]
    dataset = "DRIVE"
    ...
    IoU_calculator = Dice(n_class)

    # データ読み込み+初期設定
    #train_loader, val_loader, test_loader = dataload()
    train_loader, val_loader = dataload_DRIVE()

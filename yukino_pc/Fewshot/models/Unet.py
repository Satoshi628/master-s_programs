...
class Contrastive_head(nn.Module):
    ...
    def __init__(self, in_channels, featrue_channels):
        super().__init__()
        self.contrastive_conv = nn.Sequential(
            nn.Conv2d(in_channels, featrue_channels, kernel_size=1, padding=0),
            nn.BatchNorm2d(featrue_channels)
        )
...
class UNet_CPC(nn.Module):
    def __init__(self, in_channels, n_classes, channel=32, bilinear=True):
        super(UNet_CPC, self).__init__()
        self.in_channels = in_channels
        self.n_classes = n_classes
        self.bilinear = bilinear
        ...
        self.feat_head = Contrastive_head(channel*16 // factor, channel*16 // factor)
    ...
    def forward(self, x, train=False):
        ...
        feat1 = x5[:, :, :, x5.shape[2] // 2 :]
        feat2 = self.feat_head(x5[:, :, :, : x5.shape[2] // 2])
        x = self.up1(x5, x4)
        x = self.up2(x, x3)
        x = self.up3(x, x2)
        x = self.up4(x, x1)

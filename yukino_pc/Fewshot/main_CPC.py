...
from utils.loss import IoULoss, get_class_rate_weight, get_class_balancing_weight, Contrastive_Loss_moepy, CPC_Loss
from models.Unet import UNet_CPC
...
def train(cfg, model, train_loader, criterion, optimizer, eval_calculater, device):
    ...
    for batch_idx, (inputs, targets) in tqdm(enumerate(train_loader), total=len(train_loader), leave=False):
        ...
        elif cfg.train_conf.use_Loss == "CPC":
...
def main(cfg):
    ...
    # モデル設定
    if model_name == "U-Net-CPC":
        model = UNet_CPC(in_channels=in_channel, n_classes=n_class, channel=32).cuda(device)

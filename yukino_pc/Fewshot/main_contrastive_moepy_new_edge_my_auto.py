...
from utils.loss import IoULoss, get_class_rate_weight, get_class_balancing_weight, Contrastive_Loss_moepy, Contrastive_Loss_moepy_auto
...
############## train関数 ##############
def train(model, train_loader, criterion, optimizer, optimizer_cri, eval_calculater, device):
    # モデル→学習モード
    model.train()
    criterion.train()
...
def main(cfg):
    ...
    optimizer_cri = torch.optim.Adam(criterion.parameters(), lr=cfg.scheduler.max_lr)  # Adam
    ...
    for epoch in range(cfg.train_conf.epoch):
        # train
        train_loss, train_result = train(model, train_loader, criterion, optimizer, optimizer_cri, eval_calculater, device)
        ...
        with open(PATH_2, mode='a') as f:
            ...
            for v in val_result.values():
                f.write(f"{v}\t")
            f.write("{}".format(criterion.kappa.item()))
            f.write("\n")

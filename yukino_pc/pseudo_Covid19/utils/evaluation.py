...
class IoU():
    ...
    def update(self, outputs, targets, mode='softmax'):
        ...
        #one hot vector にしたいため、targetsのignoreをself.__classificationの値にする
        targets[targets == IGNORE] = self.__classification
        #one hot vector化
        pre_one_hot_vector = one_hot_changer(predicted, self.__classification + 1, dim=1, bool_=True)
    ...
    def __call__(self):#IoUとmean_IoUが出力される。
        IoU = (self.__Intersection / (self.__union + 1e-24)).float()[:-1]
        mean_IoU = IoU.mean()
...
class Pseudo_accracy():
    ...
    def update(self, outputs, targets):
        ...
        TP = (pre_one_hot_vector & label_one_hot_vector).sum(dim=(0, 2, 3))
        self.__TP += TP
        self.__FP += pre_one_hot_vector.sum(dim=(0, 2, 3)) - TP
        self.__FN += label_one_hot_vector.sum(dim=(0, 2, 3)) - TP

    def __call__(self):
        """各クラスのAccuracy,Precition,Recall,F1を返す

        Returns:
            [dict]: 各クラスのkeyにはAccuracy,Precition,Recall,F1が入る。
        """        

        Accuracy = self.__TP / (self.__TP + self.__FP + self.__FN + 1e-9)
        Precition = self.__TP / (self.__TP + self.__FP + 1e-9)
        Recall = self.__TP / (self.__TP + self.__FN + 1e-9)
        F1_score = 2 * Precition * Recall / (Precition + Recall + 1e-9)
        
        Accuracy[-1] = Accuracy[:-1].mean()
        Precition[-1] = Precition[:-1].mean()
        Recall[-1] = Recall[:-1].mean()
        F1_score[-1] = F1_score[:-1].mean()

        acc_dict = {
            "Accuracy": Accuracy.to("cpu").tolist(),
            "Precition": Precition.to("cpu").tolist(),
            "Recall": Recall.to("cpu").tolist(),
            "F1_score": F1_score.to("cpu").tolist()
        }
        self._reset()

...
def dataload(pseudo_path=None):
    ...
    train_transform = ut.Compose([#ut.RandomCrop(size=(256, 256)),  # ランダムクロップ(size=320×320)
                                    ...
                                    #ut.ColorJitter(brightness=0.2, contrast=0.2, saturation=0.2, hue=0.2),
                                    ut.ToTensor(),  # 0~255 ==> 0~1 + Tensor型に変換
                                    #ut.RandomErasing1(p=0.25), 
...
def train():
    ...
    # 初期設定
    sum_loss = 0 #softmax cross entropy loss
...
if __name__ == '__main__':
    ...
    criterion = nn.CrossEntropyLoss(ignore_index=IGNORE)  #Softmax Cross Entropy Loss

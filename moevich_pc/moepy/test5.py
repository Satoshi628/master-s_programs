...
def dataload():
    ...
    test_transform1 = ut.Compose([ut.ToTensor(), #0~1正規化+Tensor型に変換
                                 ut.Normalize(mean=[0.485, 0.456, 0.406],std=[0.229, 0.224, 0.225]), #データの標準化
                                 ])
    test_transform2 = ut.Compose([ut.ToTensor(), #0~1正規化+Tensor型に変換
                                 ut.Normalize(mean=[0.485, 0.456, 0.406],std=[0.229, 0.224, 0.225]), #データの標準化
                                 ])
    ...
    test_dataset1 = Covid19_Loader(dataset_type='test', transform=test_transform1)
    test_dataset2 = Covid19_Loader(dataset_type='test', transform=test_transform2)

    # dataloader作成  batch_size=ミニバッチサイズ, shuffle(=True)=データをシャッフル,drop_last(=True)=ミニバッチサイズに合うようにデータを補う

    test_loader1 = torch.utils.data.DataLoader(test_dataset1, batch_size=args.Tbatchsize, shuffle=False, drop_last=True)
    test_loader2 = torch.utils.data.DataLoader(test_dataset2, batch_size=args.Tbatchsize, shuffle=False, drop_last=True)
...
def test():
    # モデル→推論モード
    model.eval()
    visual = IoU_image([[0,0,0],[255,0,0],[0,255,0],[0,0,255]],
                root_dir=f"{args.out}/images",
                )
    ...
    with torch.no_grad():
        ...
        for batch_idx, ((inputs1, targets1), (inputs2, targets2)) in enumerate(tqdm(zip(test_loader1,test_loader2), leave=False)):
            # GPUモード  
            #inputs = inputs.cuda(device)
            #targets = targets.cuda(device)
            inputs1 = inputs1.cuda(device)
            targets1 = targets1.cuda(device)
            inputs2 = inputs2.cuda(device)
            targets2 = targets2.cuda(device)
            ...
            # 出力をsoftmax関数に(0~1)
            output = F.softmax(yy, dim=1)
            # 最大ベクトル
            _, predicted = output.max(1)
            
            #可視化

            visual.save_fig(inputs2, mode="input")
            visual.save_fig(targets2, mode="label")
            ...
            """
            # total = 正解, correct = 予測 
            total += (targets2.size(0)*targets2.size(1)*targets2.size(2))
            correct += predicted.eq(targets2).sum().item()  #predicted.eq(targets) : 教師ラベルと一致していたらTrue, 不一致ならFalse
...
if __name__ == '__main__':
    ...
    # モデル設定
    model = UNet_4(3,4).cuda(device)


    # データ読み込み+初期設定 
    test_loader1, test_loader2 = dataload()
    ...
    model_path = f"{args.out}/model.pth"
    print(model_path)
    model.load_state_dict(torch.load(model_path))

    ##### test #####
    test_accuracy, iou, miou = test()

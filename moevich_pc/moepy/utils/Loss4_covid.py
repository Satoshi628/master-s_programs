...
class SquareLoss(torch.nn.Module):
    ...
    def forward(self, bh_0, bh_1, bh_2, bh_3, btargets, y):   # output 出力(64次元)  targets ラベル
        ...
        for h_0, h_1, h_2, h_3, targets in zip(bh_0, bh_1, bh_2, bh_3, btargets):
            errorflag = [False, False, False, False]
            #errorflag = [False, False, False]

            a0 = h_0.permute(1, 2, 0)[targets == 0] #a0にclass0のピクセルのみを入れる
            #print("a0", a0.shape)
            a1 = h_1.permute(1, 2, 0)[targets == 1] #a1にclass1のピクセルのみ入れる
            a2 = h_2.permute(1, 2, 0)[targets == 2] #a2にclass2のピクセルのみ入れる
            a3 = h_3.permute(1, 2, 0)[targets == 3] #a3にclass3のピクセルのみ入れる
            ...
            h_0 = h_0.view(1, h_0.size(0), h, w) #[1, 個数, c, 256, 256]
            h_1 = h_1.view(1, h_1.size(0), h, w)
            h_2 = h_2.view(1, h_2.size(0), h, w)
            h_3 = h_3.view(1, h_3.size(0), h, w)
            ...
            h_0 = F.normalize(h_0, dim=1)  #channelに対して行う
            h_1 = F.normalize(h_1, dim=1)  
            h_2 = F.normalize(h_2, dim=1)  
            h_3 = F.normalize(h_3, dim=1)

            #選んだ点(a0～a3)と特徴マップ(h_0～h_1)を演算(cos類似度の計算)
            inner0 = (y0 * h_0).sum(dim=1)
            inner1 = (y1 * h_1).sum(dim=1)
            inner2 = (y2 * h_2).sum(dim=1)
            inner3 = (y3 * h_3).sum(dim=1)
            ...
            loss0_sum = loss0_sum + loss0
            loss1_sum = loss1_sum + loss1
            loss2_sum = loss2_sum + loss2
            loss3_sum = loss3_sum + loss3
            loss4_sum = loss4_sum + loss4

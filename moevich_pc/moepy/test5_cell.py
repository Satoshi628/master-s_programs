...
def dataload():
    ...
    test_dataset1 = Cell_Dataset_Loader(dataset_type='test', transform=test_transform1)
    test_dataset2 = Cell_Dataset_Loader(dataset_type='test', transform=test_transform2)
    ...
    with torch.no_grad():
        ...
        for batch_idx, ((inputs1, targets1), (inputs2, targets2)) in enumerate(tqdm(zip(test_loader1,test_loader2), leave=False)):
            ...
            for i in range(predicted.size(0)):
                num += 1
                item = targets1[i]
                img = np.zeros((256, 256, 3)) # <- 3はRGBの3
                for k in range(256):
                    for l in range(256):
                        if (item[k][l] == 0):
                            img[k][l] = (0, 0, 0)
...
if __name__ == '__main__':
    ...
    model_path ="result181/model.pth"
    model.load_state_dict(torch.load(model_path))

...
from utils.dataset import Eye_Loader, Eye_Loader_traintest, Eye_Loader_simCLR, Eye_Loader_ARMS2
from models.resnet2D import resnet2D50, resnet2D18, resnet2D18_atten, resnet2D18_simCLR
...
def dataload(cfg):
    ...
    train_dataset1 = Eye_Loader_simCLR(root_dir=cfg.root_dir, dataset_type='train', split=cfg.split, use_age=cfg.use_age, use_sex=cfg.use_sex, use_mask=cfg.use_mask)
...
############## pre train関数 ##############
def pre_train(model, train_loader, criterion, optimizer, device):
    # モデル→学習モード
    model.train()
    ...
    for batch_idx, (inputs1, inputs2, targets) in tqdm(enumerate(train_loader), total=len(train_loader), leave=False):
        #GPUモード高速化
        ...
        # 入力画像をモデルに入力
        out = model.pre_trainning(inputs)
...
def main(cfg):
    ...
    for epoch in range(cfg.train_conf.epoch):
        scheduler.step()
        loss = pre_train(model, train_loader1, criterion, optimizer, device)
        print(f"Epoch{epoch + 1:3d}/{cfg.train_conf.epoch:3d} contrastive Loss:{loss:.5f}")
    ...
    for epoch in range(cfg.train_conf.epoch):
        ...
        # train
        train_loss, train_acc = train(model, train_loader2, criterion, optimizer, device)

...
class ViT_with_AdNet(VisionTransformer):
    """ Vision Transformer with support for patch or hybrid CNN input stage
    """
    ...
    def __init__(self, img_size=224, patch_size=16, in_chans=3, num_classes=1000, embed_dim=768, depth=12,
                 ...
                 drop_path_rate=0., norm_layer=nn.LayerNorm, init_values=None,
                 use_abs_pos_emb=True, use_rel_pos_bias=False, use_shared_rel_pos_bias=False,
                 use_mean_pooling=True, init_scale=0.001, max_iter=10000):
        super().__init__(img_size, patch_size, in_chans, num_classes, embed_dim, depth,
                         ...
                         drop_path_rate, norm_layer, init_values,
                         use_abs_pos_emb, use_rel_pos_bias, use_shared_rel_pos_bias,
                         use_mean_pooling, init_scale)

        self.ad34net = AdversarialNetwork(embed_dim, embed_dim//4, max_iter)
...
@register_model
def beit_base_patch16_224_with_AdNet(pretrained=False, **kwargs):
    model = ViT_with_AdNet(
        patch_size=16, embed_dim=768, depth=12, num_heads=12, mlp_ratio=4, qkv_bias=True,
        norm_layer=partial(nn.LayerNorm, eps=1e-6), **kwargs)
    model.default_cfg = _cfg()
    return model
...
def load_state_dict(model, state_dict, ignore_missing="relative_position_index"):
    missing_keys = []
    unexpected_keys = []
    error_msgs = []

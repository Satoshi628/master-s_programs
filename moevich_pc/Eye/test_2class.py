...
from models.resnet2D import resnet2D2_18, resnet2D2_50, ResNet2D_2, resnet2D18_siam
...
def main(cfg):
    ...
    # model load
    model_path = "result/model_ARMS2.pth"
    model.load_state_dict(torch.load(model_path))

    # CAM
    test_ARMS2_acc1, test_CFH_acc1, test_all_acc1 = test(model, test_loader, device)

    # model load
    model_path = "result/model_CFH.pth"
    model.load_state_dict(torch.load(model_path))

    # CAM
    test_ARMS2_acc2, test_CFH_acc2, test_all_acc2 = test(model, test_loader, device)

    test_ARMS2_acc = max(test_ARMS2_acc1,test_ARMS2_acc2)
    test_CFH_acc = max(test_CFH_acc1,test_CFH_acc2)
    test_all_acc = max(test_all_acc1,test_all_acc2)

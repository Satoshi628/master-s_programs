...
#----- My Module -----#
from utils.utils import Multi_CT_ndarrayToTensor
...
def dataload(cfg):
    ...
    train_transform = Compose([Multi_CT_ndarrayToTensor(WW=[256,1024,100], WL=[0,0,50]), # [0,1] + Tensor型に変換
                                #Resize([256,256]),
                                RandomCrop(size=(416, 480)),  # random crop(512x512 -> 512x512)
                                Pad([16,48,16,48]),
                                RandomHorizontalFlip(p=0.5), # left right flip(p=probability)
                                RandomVerticalFlip(p=0.5), # up down flip(p=probability)
                                ])

    test_transform = Compose([Multi_CT_ndarrayToTensor(WW=[256,1024,100], WL=[0,0,50]),  # [0,1] + Tensor型に変換
                                #Resize([256,256])
                                ])
...
def main(cfg):
    ...
    with open(PATH_2, mode='w') as f:
        f.write("epoch\ttestidation loss\ttest acc\n")

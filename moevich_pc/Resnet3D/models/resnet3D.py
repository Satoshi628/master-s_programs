...
class ResidualBlock_Small(nn.Module):
    ...
    def __init__(self, in_channel, middle_channel, stride=1):
        ...
        self.conv = nn.Sequential(
            nn.Conv3d(in_channel, middle_channel, kernel_size=(1, 3, 3), stride=stride, padding=pad, bias=False),
            nn.BatchNorm3d(middle_channel),
            nn.ReLU(inplace=True),
            nn.Conv3d(middle_channel, middle_channel, kernel_size=(1, 3, 3), stride=1, padding=1, bias=False),
            nn.BatchNorm3d(middle_channel)
        )
        ...
        if stride != 1 or in_channel != middle_channel:
            self.shortcut = nn.Sequential(
                nn.Conv3d(in_channel, middle_channel, kernel_size=1, stride=stride, padding=sc_pad, bias=False),
                nn.BatchNorm3d(middle_channel)
            )
...
class ResidualBlock_Large(nn.Module):
    ...
    def __init__(self, in_channel, middle_channel, stride=1):
        ...
        self.conv = nn.Sequential(
            nn.Conv3d(in_channel, middle_channel, kernel_size=(1, 1, 1), bias=False),
            nn.BatchNorm3d(middle_channel),
            nn.ReLU(inplace=True),
            nn.Conv3d(middle_channel, middle_channel, kernel_size=(1, 3, 3), stride=stride, padding=pad, bias=False),
            nn.BatchNorm3d(middle_channel),
            nn.ReLU(inplace=True),
            nn.Dropout3d(0.2),
            nn.Conv3d(middle_channel, middle_channel * 4, kernel_size=(1, 1, 1), bias=False),
            nn.BatchNorm3d(middle_channel * 4)
        )
        if in_channel == middle_channel * 4:
            self.shortcut = nn.Sequential()
        else:
            self.shortcut = nn.Sequential(
                nn.Conv3d(in_channel, middle_channel * 4, kernel_size=1, stride=stride, padding=sc_pad, bias=False),
                nn.BatchNorm3d(middle_channel * 4)
            )
...
class ResNet3D(nn.Module):
    ...
    def __init__(self, block, num_blocks, non_local, in_c=3, num_classes=1):
        ...
        self.conv = nn.Sequential(
            nn.Conv3d(in_c, channel, kernel_size=(1, 7, 7), stride=2, padding=(0, 3, 3), bias=False),

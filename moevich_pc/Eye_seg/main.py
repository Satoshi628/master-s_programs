...
from utils.dataset import IDRID_Loader
...
def dataload(dataset_name, cfg):
    ...
    train_transform = ut.Compose([ut.RandomCrop(size=(608, 928)),  # ランダムクロップ(size=512x1024)
    ...
    val_transform = ut.Compose([ut.CenterCrop(size=(928, 608)),
                                ut.ToTensor(),  # 0~255 ==> 0~1 + Tensor型に変換
                                ut.Normalize(mean=[0.485, 0.456, 0.406], std=[0.229, 0.224, 0.225]),  # データ標準化
                                ])

    if dataset_name == "IDRID":
        train_dataset = IDRID_Loader(root_dir=cfg.root_dir, dataset_type='train', transform=train_transform)
        val_dataset = IDRID_Loader(root_dir=cfg.root_dir, dataset_type='test', transform=val_transform)

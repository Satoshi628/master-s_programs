...
class IoU_image():
    ...
    def save_fig(self, tensor, path, mode="input"):
        """入力されたtensorを可視化する関数
        ...
        if mode == "input":
            ...
            tensor = tensor[:,:,:,[2,1,0]]
            for image, p in zip(tensor, path):
                self.plot_image(image, p, mode)
        ...
        if mode == "label":
            ...
            for image, p in zip(seg_image, path):
    ...
    def plot_image(self, image, path, mode):
        
        cv2.imwrite("{}/case{:04}.png".format(self.root_dir, path), image)

...
def dataload(cfg):
    ...
    train_transform = Compose([CT_ndarrayToTensor(WW=256, WL=0), # [0,1] + Tensor型に変換
                                #RandomCrop(size=(256, 256)),  # random crop(512x512 -> 256x256)
                                RandomHorizontalFlip(p=0.5), # left right flip(p=probability)
                                RandomVerticalFlip(p=0.5), # up down flip(p=probability)
                                ])

    test_transform = Compose([CT_ndarrayToTensor(WW=256, WL=0),  # [0,1] + Tensor型に変換
                            ])

    train_dataset = PVT_Loader(root_dir=cfg.root_dir, dataset_type='train', transform=train_transform)
    test_dataset = PVT_Loader(root_dir=cfg.root_dir, dataset_type='test', transform=test_transform)
...
def train(model, train_loader, criterion, optimizer, device):
    ...
    for batch_idx, (inputs, targets) in tqdm(enumerate(train_loader), total=len(train_loader), leave=False):
        ...
        ###精度の計算###
        total += output.size(0)
        correct += (targets == torch.round(torch.sigmoid(output[:,0]))).sum()
...
def main(cfg):
    ...
    with open(PATH_2, mode='w') as f:
        f.write("epoch\ttestidation loss\tmIoU\n")

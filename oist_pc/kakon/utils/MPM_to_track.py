...
def global_association_solver(track):
    ...
    for idx, uid in enumerate(uni_ids):
        ...
        vec_norm_th1 = alpha[0] * torch.clamp(diff_time, min=1) * torch.norm(move_vector[idx: idx + 1], dim=-1)
        vec_norm_th2 = alpha[1] * torch.clamp(diff_time, min=1) * torch.norm(move_vector[idx: idx + 1], dim=-1)

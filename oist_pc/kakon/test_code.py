...
def feature_residual(img1, img2, img3, gray_latest):
    ...
    residual_map, bboxes = model(img1[None], img2[None], None, None)
...
# image_paths = sorted(glob.glob(os.path.join("/mnt/kamiya/code/my_exp/kakon/video_images/4x_sample1", "*")))
image_paths = sorted(glob.glob(os.path.join("/mnt/kamiya/dataset/root/data/sample6/video", "*")))
...
inputs1 = im_read(image_paths[39]).cuda()
inputs2 = im_read(image_paths[40]).cuda()
inputs3 = im_read(image_paths[41]).cuda()
# gray_latest = im_read(gray_img_paths[31], GRAY=True).cuda()
# feature_residual(inputs1, inputs2, inputs3, gray_latest)

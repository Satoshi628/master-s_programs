...
def test(cfg, model, test_loader, tracker, device):
    ...
    tracks = np.concatenate([tracks[:, :2], tracks[:, 2:4], tracks[:, 2:4] + 12, tracks[:, -1:]], axis=-1)
...
############## test function ##############
def reverse_test(cfg, model, test_loader, tracker, device):
    #なぜかevalモードだとうまくいかない
    model.eval()
    detection = []
    tracks = []
    ...
    with torch.no_grad():
        for batch_idx, inputs in tqdm(enumerate(test_loader), total=len(test_loader), leave=False):
            ...
            times = np.full([save_det.shape[0], 1], batch_idx-1, dtype=np.int32)
            detection.append(np.concatenate([times, save_det], axis=-1))
            detects.append(detect[:, [0, 1, 4]].cpu())
    ...
    tracker.update(detects[::-1])


    tracks = tracker(len(test_loader.dataset.get_videos()), delay=1)  #.cpu().numpy()
    tracks[:, 0] = tracks[:, 0].max() - tracks[:, 0]
    _, indices = torch.sort(tracks[:,0])
...
def main(cfg):
    ...
    tracker = MPM_to_Track_kalman(move_speed=[11.77])

...
class DoubleConv(nn.Module):
    ...
    def __init__(self, in_channels, out_channels, mid_channels=None):
        ...
        self.double_conv = nn.Sequential(
            nn.Conv3d(in_channels, mid_channels, kernel_size=3, padding=1, bias=False, padding_mode='replicate'),
            nn.BatchNorm3d(mid_channels),
            Mish(),
            nn.Conv3d(mid_channels, out_channels, kernel_size=3, padding=1, bias=False, padding_mode='replicate'),
...
class UNet_3D(nn.Module):
    def __init__(self, in_channels, n_classes, channel=32,
        ...
        self.outc = nn.Conv3d(channel, n_classes, kernel_size=1, bias=True, padding_mode='replicate')
    ...
    def compare_feature(self, x, coord):
        ...
        x = self.up2(x, x3)
        x = self.up3(x, x2)
        feature1 = self.up4(x, x1)
        out = self.outc(feature1)
        out = torch.sigmoid(out)
...
class UNet_3D_FA(nn.Module):
    ...
    def get_coord(self, x):
        ...
        out = out.flatten(0, 2)
        h = (out >= self.noise_strength) * (out == self.maxpool(out)) * 1.0
        for _ in range(3):
            h = self.gaussian_filter(h)
            h = (h != 0) * (h == self.maxpool(h)) * 1.0
        ...
        for batch in range(h.size(0)):
            coord = coordinate[coordinate[:, 0] == batch]
            score = out[coord[:, 0], coord[:, 1], coord[:, 2]]
            coord = torch.cat([coord[:, [2, 1]], score[:, None]], dim=-1)
            coord_list.append(coord.to("cpu").numpy())
        return coord_list
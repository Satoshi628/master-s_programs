...
class Sine_Positional_Encoding(nn.Module):
    def __init__(self, d_model, temperature=10000, **kwargs):
        super().__init__()
        if d_model % 2 != 0:
            raise ValueError("d_modelは偶数でなければなりません")
        self.d_model = d_model // 2
        self.temperature = temperature

    def forward(self, coordinate, size):
        """coordinateからSine Positional Encodingを作製する
        参考資料:https://qiita.com/halhorn/items/c91497522be27bde17ce#positional-encoding

        Args:
            coordinate (tensor[batch,T,num,2(x,y)], long): batchに分けられた検出位置tensor
...
class Learned_Positional_Encoding(nn.Module):
    ...
    def forward(self, coordinate, size):
        ...
        Args:
            coordinate (tensor[batch,T,num,2(x,y)], long): maskのかかった部分は-1の値になる。
            size (list[2(H, W)]): input image size

        Returns:
            tensor[batch,num,d_model]: maskのかかった部分は0の値になる。
        """        
...
class Linear_Positional_Encoding(nn.Module):
    ...
    def forward(self, coordinate, size):
        """coordinateからlinear Positional Encodingを作製する
        参考資料:https://github.com/magicleap/SuperGluePretrainedNetwork/blob/c0626d58c843ee0464b0fa1dd4de4059bfae0ab4/models/superglue.py#L73

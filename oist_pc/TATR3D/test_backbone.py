...
def dataload(cfg):
    ...
    #OIST
    if cfg.dataset == "OIST":
        test_dataset = OISTLoader(root_dir="/mnt/kamiya/dataset/OIST/LowDensity_mov", mode="test", Staning="Low", split=cfg.OIST.split, length=cfg.OIST.length, transform=test_transform)
...
def main(cfg):
    ...
    #評価指標計算クラス定義
    test_function = Object_Detection(mode="maximul", noise_strength=cfg.parameter.noise_strength, pool_range=11)

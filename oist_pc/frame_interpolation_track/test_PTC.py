...
def dataload(cfg):
    ...
    test_dataset = PTC_Loader(root_dir=cfg.OIST.root_dir, Molecule=cfg.OIST.Staining, Density="Low", mode="test", split=cfg.OIST.split, length=16, delay=1, transform=test_transform)
    #move_speed_max = PTC_Loader(Staining="GFP-Mid", mode="train", split=0, length=16).get_max_move_speed()
...
def test(model, test_loader, Track_transfor, Tracking_Eva, device, cfg):
    ...
    track = Track_transfor(test_loader.dataset.image_length, delay=cfg.parameter.delay)

...
def dataload(cfg):
    ...
    train_dataset = OIST_Loader_Train30(root_dir=cfg.OIST.root_dir, Staining=cfg.OIST.Staining, mode="train", split=cfg.OIST.split, length=16, delay=2, transform=train_transform)
    val_dataset = OIST_Loader_Train30(root_dir=cfg.OIST.root_dir, Staining=cfg.OIST.Staining, mode="val", split=cfg.OIST.split, length=16, delay=2, transform=val_transform)
...
def val(model, val_loader, criterion, device, cfg):
    ...
    detection_calc_w_img = Object_Detection(right_range=10, noise_strength=cfg.parameter.noise_strength)
    detection_calc_wo_img = Object_Detection(right_range=10, noise_strength=cfg.parameter.noise_strength)
    ...
    with torch.no_grad():
        for batch_idx, (inputs, targets, point) in tqdm(enumerate(val_loader), total=len(val_loader), leave=False):
            ...
            detection_calc_w_img.calculate(coord[::2], point.flatten(0, 1)[:, ::2, -2:])
            detection_calc_wo_img.calculate(coord[1::2], point.flatten(0, 1)[:, 1::2, -2:])
    
    acc = detection_calc_w_img()
    print("with image object accuracy")
    print(acc)
    acc = detection_calc_wo_img()
    print("without image object accuracy")
    print(acc)

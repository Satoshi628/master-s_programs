...
from utils.loss import KLDiv_Loss
...
from models.Unet_3D import UNet_3D, UNet_3D_multi
...
def dataload(cfg):
    ...
    train_dataset = OIST_Loader_Train30_2(root_dir=cfg.OIST.root_dir, Staining=cfg.OIST.Staining, mode="train", split=cfg.OIST.split, length=16, delay=cfg.parameter.delay, transform=train_transform)
    val_dataset = OIST_Loader_Train30_2(root_dir=cfg.OIST.root_dir, Staining=cfg.OIST.Staining, mode="val", split=cfg.OIST.split, length=16, delay=cfg.parameter.delay, transform=val_transform)
...
def train(model, train_loader, criterion, criterion2, optimizer, device):
    ...
    for batch_idx, (inputs, targets, point) in tqdm(enumerate(train_loader), total=len(train_loader), leave=False):
        ...
        output = model(inputs)
        loss = criterion(output.sum(dim=1, keepdim=True), targets)
        # KLdiv = criterion2(output)
        # loss = loss - 5e-4 * KLdiv
...
############## validation function ##############
def val(model, val_loader, criterion, criterion2, device, cfg):
    model.eval()
    coord_F = None
    # Determinism
    torch.backends.cudnn.deterministic = True
...
def main(cfg):
    ...
    for epoch in range(cfg.parameter.epoch):
        ...
        # validation
        val_loss, val_acc = val(model, val_loader, criterion, criterion2, device, cfg)

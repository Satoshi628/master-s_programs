...
class OIST_Loader_Train30_2(data.Dataset):
    # 初期設定
    def __init__(self, root_dir="/mnt/kamiya/dataset/OIST2", Staining="GFP", mode="train", split=0, length=16, delay=2, transform=None):
        # 30FPS => 120FPS case delay=4
        folder_dict = {"GFP": "GFP_v9_Mid", "TMR": "TMR_v9_Mid", "SF650": "SF650_v9_Mid"}
        use_data_list = {
                        "GFP": ["1","3","2","4","5","6","7","8","9","10"],
                        "TMR": ["1","3","2","4","5","6","7","8","9","10"],
                        "SF650": ["1","3","2","4","5","6","7","8","9","10"],}
        
        split_list = [{"train": [0, 1, 2, 3, 4, 5, 6, 7], "val": [8], "test": [9]},
                      {"train": [9, 0, 1, 2, 3, 4, 5, 6], "val": [7], "test": [8]},
                      {"train": [8, 9, 0, 1, 2, 3, 4, 5], "val": [6], "test": [7]},
                      {"train": [7, 8, 9, 0, 1, 2, 3, 4], "val": [5], "test": [6]},
                      {"train": [6, 7, 8, 9, 0, 1, 2, 3], "val": [4], "test": [5]}]
        ...
        #パス、使用フレームを選択
        use_paths = [os.path.join(root_dir, folder_dict[Staining], use_data_list[Staining][idx]) for idx in split_list[split][mode]]
        ...
        #Cell Point保存
        CP_data = [np.loadtxt(os.path.join(path, "det.txt"), delimiter=',') for path in use_paths]
...
class PTC_Loader(data.Dataset):
    # 初期設定
    def __init__(self, root_dir="/mnt/kamiya/dataset/PTC", Molecule="RECEPTOR", Density="Low", mode="train", split=0, length=16, delay=2, transform=None):
        use_data_list = {
                        "Low":
                            ["Sample1",
                            "Sample2",

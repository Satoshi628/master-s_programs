...
class KLDiv_Loss(nn.Module):
    ...
    def forward(self, predicts):
        ...
        log_pre1 = torch.clamp(pre1, 1e-4).log()
        log_pre2 = torch.clamp(pre2, 1e-4).log()
        
        KLDiv1 = pre1 * (log_pre1 - log_pre2)
        KLDiv2 = pre2 * (log_pre2 - log_pre1)
        
        loss = (KLDiv1.mean() + KLDiv2.mean()) / 2.

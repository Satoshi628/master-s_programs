...
def dataload(cfg):
    ...
    test_dataset = OIST_Loader_Test30FPS(root_dir=cfg.OIST.root_dir, Staining=cfg.OIST.Staining, mode="test", split=cfg.OIST.split, length=16, transform=test_transform)
...
def test(model, test_loader, Track_transfor, Tracking_Eva, device, cfg):
    ...
    with torch.no_grad():
        for batch_idx, (inputs, targets, point) in tqdm(enumerate(test_loader), total=len(test_loader), leave=False):
            ...
            detection_calc_w_img.calculate(coordinate[::2], point.flatten(0, 1)[::2, :, -2:])
            detection_calc_wo_img.calculate(coordinate[1::2], point.flatten(0, 1)[1::2, :, -2:])
    ...
    det_acc = detection_calc_all()
    print("all object accuracy")
    print(det_acc)
    det_acc = detection_calc_w_img()
    print("with image object accuracy")
    print(det_acc)
    det_acc = detection_calc_wo_img()

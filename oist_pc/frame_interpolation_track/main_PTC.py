...
def dataload(cfg):
    ...
    train_dataset = PTC_Loader(root_dir=cfg.OIST.root_dir, Molecule=cfg.OIST.Staining, Density="Mid", mode="train", split=cfg.OIST.split, length=16, delay=cfg.parameter.delay, transform=train_transform)
    val_dataset = PTC_Loader(root_dir=cfg.OIST.root_dir, Molecule=cfg.OIST.Staining, Density="Mid", mode="val", split=cfg.OIST.split, length=16, delay=cfg.parameter.delay, transform=val_transform)
...
def train(model, train_loader, criterion, criterion2, optimizer, device):
    ...
    for batch_idx, (inputs, targets, point) in tqdm(enumerate(train_loader), total=len(train_loader), leave=False):
        ...
        output = model(inputs)
        loss = criterion(output.sum(dim=1), targets)

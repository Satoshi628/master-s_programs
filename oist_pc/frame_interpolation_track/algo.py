...
from utils.dataset import OIST_Loader_Test30FPS, collate_delete_PAD
import utils.transforms as tf
from utils.utils import get_movelimit
from utils.MPM_to_track import MPM_to_Track_detection_copy, MPM_to_Track2, global_association_solver
...
def dataload(cfg):
    ...
    test_dataset = OIST_Loader_Test30FPS(root_dir=cfg.OIST.root_dir, Staining=cfg.OIST.Staining, mode="test", split=0, length=16, transform=test_transform)
...
def test(model, test_loader, Track_transfor, Tracking_Eva, device):
    ...
    global_association_solver(track)
...
def main(cfg):
    ...
    Track_transfor = MPM_to_Track_detection_copy(noise_strength=0.4, move_speed=move_speed_max)

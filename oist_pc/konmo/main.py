...
from utils.dataset import Konmo_Loader
import utils.transforms_seg as tf
from utils.evaluation import IoU
from utils.loss import IoULoss, soft_cldice
...
def dataload(cfg):
    ...
    train_transform = tf.Compose([
                                # tf.RandomResize((0.8,1.2)), #なし62.28%　あり61.46%
                                tf.RandomCrop_mask(size=(512, 512), p=0.8), #100% 62.28% 80% 62.56% 0% 61.92% 
...
def main(cfg):
    ...
    # criterion = IoULoss(2)
    criterion = soft_cldice(exclude_background=True)

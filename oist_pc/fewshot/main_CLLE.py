...
from utils.dataset_CLLE import (Covid19_Loader,
    Drosophila_Loader,
    Cityscapse_Loader,
    Drive_Loader,
    ACDC_Loader,
...
from utils.loss import IoULoss, get_class_rate_weight, get_class_balancing_weight, Contrastive_Loss_moepy, CLLE_Loss
from models.Unet import UNet_contrastive
...
############## train関数 ##############
def pre_train(cfg, model, train_loader, criterion, optimizer, eval_calculater, device):
    # モデル→学習モード
    model.train()
    ...
    for batch_idx, (inputs, JIT_inputs, targets) in tqdm(enumerate(train_loader), total=len(train_loader), leave=False):
        #GPUモード高速化
        inputs = inputs.cuda(device, non_blocking=True)
        JIT_inputs = JIT_inputs.cuda(device, non_blocking=True)
        ...
        JIT_output, JIT_feature = model(JIT_inputs, train=True)

        # 損失計算
        loss = criterion(output, JIT_output, feature, JIT_feature, targets)
...
def val(cfg, model, val_loader, criterion, eval_calculater, device):
    ...
    with torch.no_grad():
        ...
        for batch_idx, (inputs, JIT_inputs, targets) in tqdm(enumerate(val_loader), total=len(val_loader), leave=False):
            #GPUモード高速化
            inputs = inputs.cuda(device, non_blocking=True)
            JIT_inputs = JIT_inputs.cuda(device, non_blocking=True)
            targets = targets.cuda(device, non_blocking=True)
...
def main(cfg):
    ...
    elif cfg.train_conf.use_Loss == "CLLE":
        criterion = CLLE_Loss(n_class,
        weight=weight)
    else:
    ...
    for epoch in range(100):
        # train
        train_loss, train_result = pre_train(cfg, model, train_loader, criterion, optimizer, eval_calculater, device)
        scheduler.step()
        Break_Time()
    ...
    for epoch in range(cfg.train_conf.epoch):
        # train
        train_loss, train_result = train(cfg, model, train_loader, criterion_CE, optimizer, eval_calculater, device)

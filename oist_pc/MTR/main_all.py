...
from utils.loss import movement_Loss2, Contrastive_Loss
from utils.dataset import OISTLoader_MTR, OISTLoader_add, collate_delete_PAD
...
def dataload(cfg):
    ...
    train_dataset = OISTLoader_MTR(root_dir=cfg.OIST.root_dir, mode="train", data="GFP_im_Mid", length=16, transform=train_transform)
    val_dataset = OISTLoader_MTR(root_dir=cfg.OIST.root_dir, mode="val", data="GFP_im_Low", length=16, transform=val_transform)
    #val_dataset = OISTLoader_add(root_dir=cfg.OIST.root_dir, mode="val", data="GFP_im_Low", length=16, transform=val_transform)
...
############## train function ##############
def train(model, train_loader,  trans_criterion, unet_criterion, optimizer, device):
    model.train()
    # Nondeterminism
    ...
    for batch_idx, (inputs, targets, point) in tqdm(enumerate(train_loader), total=len(train_loader), leave=False):
        ...
        inputs = inputs[:,1:-1].cuda(device, non_blocking=True)
        targets = targets.cuda(device, non_blocking=True)
        point = point.cuda(device, non_blocking=True)
        coord = point[:, :, :, 1].long()

        x, feature, vector, coordinate = model(inputs, coord)
        trans_loss = trans_criterion(vector, point[:, :, :, [1, 2, 3, 4]])
        unet_loss = unet_criterion(x.squeeze(1), targets)
        loss = trans_loss + unet_loss
        optimizer.zero_grad()
        loss.backward()
...
############## validation function ##############
def val(model, val_loader,   trans_criterion, unet_criterion, device):
    model.eval()
    # Determinism
    torch.backends.cudnn.deterministic = True
    sum_loss = 0
...
def main(cfg):
    ...
    # def model
    model = MTR3(back_bone_path=cfg.parameter.back_bone,
                            ...
                            move_limit=move_limit).cuda(device)
    ...
    trans_criterion = movement_Loss2(move_limit)
    ...
    for epoch in range(cfg.parameter.epoch):
        ...
        # train
        train_loss = train(model, train_loader, trans_criterion, unet_criterion, optimizer, device)
        # validation
        if epoch >= 0:
            #Tracking_acc = val(model, val_loader, criterion, Track_transfor, tracking_fnc, device)
            val_loss = val(model, val_loader, trans_criterion, unet_criterion, device)
        else:

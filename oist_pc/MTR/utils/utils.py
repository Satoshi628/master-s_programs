...
def get_movelimit(dataset, factor=0.9):
    ...
    for track in all_tracks:
        # frame sort
        sort_idx = track[:, 0, 0].sort()[1]
        track = track[sort_idx]
        zero_point = track[track[:, 0, 0] == 0]
        aa = torch.unique(zero_point[:, 0, 1], return_counts=True)

        #frame,id,x,y
        unique_id = torch.unique(track[:, 0, 1])
        for target_id in unique_id:
            for i in range(1, 4):
                target_track = track[track[:, 0, 1] == target_id][:, 1]

...
class RandomHorizontalFlip(object):
    ...
    def __call__(self, img, label, point):
        if random.random() < self.p:
            ...
            point[:, :, 1, 0] = 2 * center - point[:, :, 1, 0]
            point[:, :, [2, 3, 4], 0] = -point[:, :, [2, 3, 4], 0]
            point[point[:, :, 1, 0] >= img.shape[-1]] = PAD_ID
...
class RandomVerticalFlip(object):
    ...
    def __call__(self, img, label, point):
        if random.random() < self.p:
            ...
            point[:, :, 1, 1] = 2 * center - point[:, :, 1, 1]
            point[:, :, [2, 3, 4], 1] = -point[:, :, [2, 3, 4], 1]
            point[point[:, :, 1, 1] >= img.shape[-2]] = PAD_ID
...
class RandomCrop(object):
    ...
    def __call__(self, img, label, point):
        ...
        #x crop
        point[:, :, 1, 0] -= j
        point[(point[:, :, 1, 0] < 0) | (point[:, :, 1, 0] >= w)] = PAD_ID
        #y crpo
        point[:, :, 1, 1] -= i
        point[(point[:, :, 1, 1] < 0) | (point[:, :, 1, 1] >= h)] = PAD_ID

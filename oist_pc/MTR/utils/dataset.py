...
class OISTLoader(data.Dataset):
    def __init__(self, root_dir, mode="train", data="GFP_im_Mid", split=0, length=16, transform=None):
        #--------------- 変更場所 ---------------#
        ...
        # 使用するframeの長さ
        use_frame_list = {
            "GFP_im_Mid": [[0, 100], [0, 100], [0, 100], [0, 100], [0, 100]],
            "GFP_im_Low": [[0, 100], [0, 100], [0, 100], [0, 100], [0, 100]]}
        ...
        CP_data = [h5py.File(f"{root_dir}/Point_Move_Annotation.hdf5", mode='r')[path][...] for path in EP_paths]
        self.CP_data_per_data = [torch.tensor(data) for data in CP_data]

        self.CP_data = [sample_data[sample_data[:, 0, 0] == frame] for sample_data in self.CP_data_per_data for frame in torch.unique(sample_data[:, 0, 0])]
...
##### OIST dataset #####
class OISTLoader_MTR(data.Dataset):
    def __init__(self, root_dir, mode="train", data="GFP_im_Mid", split=0, length=16, transform=None):
        #--------------- 変更場所 ---------------#
        # 追加したフォルダの名前を入れる。辞書のKey部分には任意の名前を入れる。
        use_data_list = {
    ...
    def __getitem__(self, index):
        ...
        for length in range(self.length):
            if data_index + length - 1 < 0:
                pre_image.append(self.images[0])
            else:
                pre_image.append(self.images[data_index + length - 1])
            
            if data_index + length + 1 >= len(self.images):
                next_image.append(self.images[-1])
            else:
                next_image.append(self.images[data_index + length + 1])
        
        pre_image = np.concatenate(pre_image, axis=0)
        next_image = np.concatenate(next_image, axis=0)

        image = np.stack([pre_image, image, next_image], axis=0)
...
def collate_delete_PAD(batch):
    ...
    sorted_idx = torch.argsort(point[:, :, :, 0, 0], dim=2, descending=True)
    point = torch.take_along_dim(point, sorted_idx[:, :, :, None, None], dim=2)
    max_point_num = (point[:, :, :, 0, 0] != PAD_ID).sum(dim=-1).max()

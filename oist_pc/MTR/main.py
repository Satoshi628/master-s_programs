...
from utils.loss import movement_Loss2
...
def dataload(cfg):
    ...
    train_dataset = OISTLoader(root_dir=cfg.OIST.root_dir, mode="train", data="GFP_im_Mid", length=16, transform=train_transform)
    val_dataset = OISTLoader(root_dir=cfg.OIST.root_dir, mode="val", data="GFP_im_Low", length=16, transform=val_transform)
...
def train(model, train_loader, criterion, optimizer, device):
    ...
    for batch_idx, (inputs, targets, point) in tqdm(enumerate(train_loader), total=len(train_loader), leave=False):
        ...
        vector, coordinate = model(inputs, coord)
        print(torch.all(coord == coordinate))
        
        loss = criterion(vector, point[:, :, :, [1, 2, 3, 4]])
...
def main(cfg):
    ...
    # def model
    model = MTR(back_bone_path=cfg.parameter.back_bone,
    ...
    criterion = movement_Loss2(move_limit)

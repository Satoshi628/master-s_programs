        ...
        for coord in coords:
            if len(self.track) != 0:  # 空配列じゃないなら
                ...
                if len(self.track) >= 2:
                    ...
                    match_flag2, dist_idx = linear_assignment(dist, self.move_speed[0])
                ...
                if new_coord.shape[0] != 0:
                    ...
                    for idx, tid in enumerate(new_id):
                        self.kalman_dict[f"{int(tid)}"] = KalmanFilter(new_coord[idx, :2].cpu().numpy())
                    new_track = torch.cat([new_id[:, None], new_coord], dim=-1)
                    self.max_track_id = new_id[-1]
                else:
                ...
                for idx, tid in enumerate(track_id):
                    self.kalman_dict[f"{int(tid)}"] = KalmanFilter(coord[idx, :2].cpu().numpy())
                track = torch.cat([track_id[:, None], coord], dim=-1)
                self.track.append(track)
                self.max_track_id = track_id[-1]
...
def global_association_solver(track):
    movement_limit1 = 11.77*1.5
    movement_limit1 = 11.77*5
    move_com_div_limit = 11.77*3
    ...
    length = length.float()
    track_move = (latest_coord[:, 2:] - first_coord[:, 2:]) ** 2
    ...
    for idx, uid in enumerate(uni_ids):
        ...
        fl_connect_flag = (fl_time_dist > 0) & (fl_time_dist > 0) & (fl_time_dist <= assosiate_time) & (fl_space_dist < movement_limit1)
        lf_connect_flag = (lf_time_dist > 0) & (lf_time_dist > 0) & (lf_time_dist <= assosiate_time) & (lf_space_dist < movement_limit1)
        #自身とのつながりは無くす
        ...
        for con_idx in connect_idx:
            # p = torch.exp(-lf_space_dist[con_idx] / (movement_limit1 * lf_time_dist[con_idx])).item()
        ...
        #         p = torch.exp(-fl_space_dist[con_idx] / (move_com_div_limit*lf_time_dist[con_idx])).sum().item()
        #         _cost = cost_vector.copy()
        #         _cost[con_idx[0]] = 1
        #         _cost[con_idx[1]] = 1
        #         _cost[track_num + idx] = 1
        ...
        #     combinations = torch.combinations(connect_idx, r=2)
        #     for con_idx in combinations:
        #         p = torch.exp(-lf_space_dist[con_idx] / (move_com_div_limit*lf_time_dist[con_idx])).sum().item()
        #         _cost = cost_vector.copy()
        ...
        #register FP
        #移動距離でノイズを判断
        p = torch.exp(-track_move[idx]/0.3).item()
        _cost = cost_vector.copy()
        _cost[idx] = 1
        _cost[track_num + idx] = 1
...
def global_association_solver(track):
    ...
    alpha = 1.2
    cos_theta = math.cos(10 / 180*math.pi) #弧度法
    ...
    for idx, uid in enumerate(uni_ids):
        ...
        dist_flag = torch.norm(last2first_vec, dim=-1) <= (alpha * torch.clamp(diff_time, min=1) * torch.norm(move_vector[idx: idx + 1], dim=-1))
        ...
        p = torch.exp(-latest_time / 5.).item()
        p = max(0.001, p)
        _cost = cost_vector.copy()
        _cost[idx] = 1
        process_dict[f"{uid}_end"] = [p, _cost]

    ...
    def __call__(self, img, label, point):
        if random.random() < self.p:
            ...
            for idx in range(len(point)):
                point[idx][:, 2] = 2 * center - point[idx][:, 2]
                
                point[idx][point[idx][:, 2] >= img.shape[-1]] = PAD_ID
...
class RandomVerticalFlip(object):
    ...
    def __call__(self, img, label, point):
        if random.random() < self.p:
            ...
            for idx in range(len(point)):
                point[idx][:, 3] = 2 * center - point[idx][:, 3]
                
                point[idx][point[idx][:, 3] >= img.shape[-2]] = PAD_ID
...
class RandomCrop(object):
    ...
    def __call__(self, img, label, point):
        ...
        for idx in range(len(point)):
            point[idx][:, 2] -= j
            point[idx][(point[idx][:, 2] < 0) | (point[idx][:, 2] >= w)] = PAD_ID
            #y crpo
            point[idx][:, 3] -= i
            point[idx][(point[idx][:, 3] < 0) | (point[idx][:, 3] >= h)] = PAD_ID

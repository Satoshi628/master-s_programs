...
class Time_Interpolate(nn.Module):
    ...
    def __init__(self, in_channels, out_channels, padding_mode="zeros"):
        super().__init__()
        # if bilinear, use the normal convolutions to reduce the number of channels
        self.up = nn.ConvTranspose2d(in_channels, in_channels, kernel_size=3, stride=1, padding=1)
        self.conv = DoubleConv(in_channels, out_channels, padding_mode=padding_mode)
...
class UNet_2D(nn.Module):
    def __init__(self, in_channels, n_classes, channel=32,
        ...
        self.outc = nn.Conv2d(channel, n_classes, kernel_size=1, bias=True, padding_mode=padding_mode)
    ...
    def coordinater(self, outputs):
        ...
        return detection  # [batch*length] [num,3(x, y, score)]
    ...
    def forward(self, x):
        ...
        out = out.reshape([b,t,self.n_classes,h,w]).permute(0,2,1,3,4)

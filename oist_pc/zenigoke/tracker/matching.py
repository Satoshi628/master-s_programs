...
def center_distance(atracks, btracks):
    ...
    if (len(atracks)>0 and isinstance(atracks[0], np.ndarray)) or (len(btracks) > 0 and isinstance(btracks[0], np.ndarray)):
        ...
        axyahs = np.array(axyahs)
        bxyahs = np.array(bxyahs)
    if axyahs.shape[0] == 0 or bxyahs.shape[0] == 0:
        return np.ones([axyahs.shape[0], bxyahs.shape[0]])
    dist = (axyahs[:, None, :2] - bxyahs[None, :, :2]) ** 2

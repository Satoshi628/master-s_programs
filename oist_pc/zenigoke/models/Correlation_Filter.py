...
class Wide_ResNet50V2_residual(nn.Module):
    def __init__(self, extract_layers=["layer2", "layer3"],
            thresh_low=0.5,
            thresh_high=0.5,
            anchor_sizes=[[96],],
            anchor_aspects=[[0.5, 0.75, 1, 1.25, 1.5],],
            iou_thresh=0.1,
            device="cuda:0"):
        super().__init__()
    ...
    def forward(self, x1, x2, x3):
        ...
        #元の画像の大きさの戻す
        bboxes_low = anchors_low / self.inter_rate[0]
        bboxes_high = anchors_high / self.inter_rate[0]

        bboxes_low = torch.cat([bboxes_low, logits_low[:, None]], dim=-1)
        bboxes_high = torch.cat([bboxes_high, logits_high[:, None]], dim=-1)

        bboxes = torch.cat([bboxes_high, bboxes_low], dim=0)

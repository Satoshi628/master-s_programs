...
class costom_loader(Dataset):
    def __init__(self, split, img_transform):
        super().__init__()
        with open(f"data/custom_dataset/annotations/test_{split}.json") as f:
            json_data = json.load(f)
        
        self.data = [f"data/custom_dataset/test_{split}/" + img_info["file_name"] for img_info in json_data['images']]

        self.seq_length = json_data['images'][0]['seq_length']

        #public_detection = np.load("data/custom_dataset/val_public_coord.npy")
        #self.public_bbox = [[np.array([coord[1] - 6, coord[2] - 6, coord[1] + 5, coord[2] + 5], dtype=np.float32)
        #                for coord in public_detection[public_detection[:, 0] == frame]]
        ...
        for anno_info in json_data['annotations']:
            frame_id = anno_info['image_id']
            track_id = anno_info['track_id']
            bbox = anno_info['bbox']
            boxes[frame_id][track_id] = np.array(bbox, dtype=np.float32)
        self.boxes = boxes
        self.transforms = Compose(make_coco_transforms('val', img_transform))
    ...
    def __getitem__(self, idx: int) -> dict:
        ...
        sample = {}
        sample['img'] = img
        #sample['dets'] = torch.tensor(self.public_bbox[idx])
        sample['orig_size'] = torch.as_tensor([int(height_orig), int(width_orig)])
        sample['size'] = torch.as_tensor([int(height), int(width)])
        sample['gt'] = self.boxes[idx]
        return sample

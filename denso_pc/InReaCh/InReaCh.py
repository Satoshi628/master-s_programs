...
FAISS_STANDARD_GPU = faiss.StandardGpuResources()
...
class InReaCh():
    ...
    def gen_channels(self, quite: bool = False):
        ...
        #ToDo:めっちゃ遅い、どうにかできないか
        self.nn_object = faiss.GpuIndexFlatL2(FAISS_STANDARD_GPU, self.patches.size(1), FAISS_CONFIG) # deterministic brute force nn
        ...
        self.nn_object.add(np.array(nominal_points))
    ...
    def predict(self, t_images: List[np.ndarray], 
        ...
        for test_img_index in tqdm.tqdm(range(t_patches.size(0)), ncols=100, desc = 'Predicting On Images', disable=quite):
            dist, ind = self.nn_object.search(torch.permute(t_patches[test_img_index],(1,0)).numpy(),1)
...
if __name__ == '__main__':
    ...
    model = load_wide_resnet_50(return_nodes=return_nodes, verbose=True)
    ...
    for class_name in class_names:
        ...
        images, masks, corr_types = load_corrupted_data(class_name=class_name, 
                                                            data_dir='/mnt/kamiya/dataset/MVtec_AD/',
                                                            num_corrupted=40)
        
        test_images, test_truths, test_class = load_testing_data(class_name=class_name, data_dir='/mnt/kamiya/dataset/MVtec_AD/')

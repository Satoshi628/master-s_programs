...
class DefectDataset(Dataset):
    ...
    def transform(self, x, img_len, binary=False):
        ...
        x = downsampling(x, (img_len[0], img_len[1]), bin=binary)
        x = x.reshape(channels, img_len[0], img_len[1])
        return x
    ...
    def __getitem__(self, index):
        if c.use_3D_dataset:
            ...
            depth = torch.zeros([1, c.depth_len[0], c.depth_len[1]])
            fg = torch.ones([1, c.depth_len[0], c.depth_len[1]])
        ...
        if self.set == 'test' and self.get_mask:
            if "good" in self.masks[index]:
                mask = np.zeros([1, *img.size()[1:]])
                mask = self.transform(mask, c.depth_len, binary=True)[:1]
                ret.append(mask)

...
def train_on_device(obj_names, args):
    ...
    for obj_name in obj_names:
        ...
        noise_gen = gn.Perlin_Noise_Generater(noise_scale_min=0, noise_scale_max=6, th = 0.7,
        ...
        train_dataset = MVTecDataset(mvtec_root="/mnt/kamiya/dataset/MVtec_AD",
        ...
        test_dataset = MVTecDataset(mvtec_root="/mnt/kamiya/dataset/MVtec_AD",
                                    dtd_root="/mnt/kamiya/dataset/DTD/images",
                                    category=obj_name,
                                    input_size=[256,256],
                                    Faster=True,
        ...
        train_dataloader = DataLoader(train_dataset, batch_size=args.bs,
                                shuffle=True,
                                num_workers=4, #57s/epoch => 50s/epoch
                                pin_memory=True
                                )
        ...
        for epoch in range(args.epochs):
            ...
            with tqdm(enumerate(train_dataloader), total=len(train_dataloader), leave=True) as pbar:
                for i_batch, sample_batched in pbar:
                    ...
                    gray_rec = model(anomaly_images)
                    l2_loss = loss_l2(gray_rec,images)
                    ssim_loss = loss_ssim(gray_rec, images)
                    ...
                    if args.visualize and n_iter % 400 == 0:
                        visualizer.visualize_image_batch(anomaly_images, n_iter, image_name='batch_augmented')
                        visualizer.visualize_image_batch(images, n_iter, image_name='batch_recon_target')

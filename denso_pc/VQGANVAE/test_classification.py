...
def test(obj_names, mvtec_path, checkpoint_path, base_model_name,saveimages):
    ...
    for obj_name in obj_names:
        ...
        model.load_state_dict(torch.load(os.path.join(checkpoint_path,run_name+"_bestscore.pckl"), map_location='cuda:0'))
        model.cuda()
        model.eval()
        ...
        with torch.no_grad():
            ...
            for i_batch, sample_batched in tqdm(enumerate(dataloader), total=len(dataloader), leave=True):
                ...
                if saveimages:
                    ...
                    # see_img(outputs,f'{savepath}/{obj_name}/',i,'rec')
                    # see_img(gray_batch,f'{savepath}/{obj_name}/',i,'orig')
                    # see_img_heatmap(gray_batch,segresult,f'{savepath}/{obj_name}/',i,'hetamap')
                    savefig(gray_batch,segresult,truemaskresult,f'{savepath}/{obj_name}/'+f'segresult{i}.png',outputs)
                    i=i+1
...
if __name__=="__main__":
    ...
    with torch.cuda.device(args.gpu_id):
        # test(["wood"], args.data_path, args.checkpoint_path, args.base_model_name,args.saveimages)

...
class DynamicApproximateGreedyCoresetSampler(GreedyCoresetSampler):
    def __init__(
            self,
            percentage: float,
            device: torch.device,
    ...
    def _compute_greedy_coreset_indices(self, features: torch.Tensor) -> np.ndarray:
        ...
        dist = coreset_distance/coreset_distance.max()
        x = np.arange(len(coreset_distance))/len(coreset_distance)
        center_dist = dist**2 + x**2
        idx = center_dist.argmin()

        return np.array(coreset_indices[:idx]), coreset_distance

...
class PatchCore(torch.nn.Module):
    ...
    def _fill_memory_bank(self, input_data):
        ...
        #特徴マップの大きさは[b,c,28,28]
        images = np.concatenate(images, axis=0)
        features = np.concatenate(features, axis=0)
        #パッチに分ける。縦と横のsplitを間違えるとfeaturesと対応できなくなるので注意
        images = np.stack(np.split(images, 28, -2), axis=1)
        images = np.stack(np.split(images, 28, -1), axis=2)

        #images.shape=[n, 3, 8, 8]
        images = images.reshape(features.shape[0], 3, 8, 8)

        features, indices = self.featuresampler.run(features)
    ...
    def _predict_dataloader(self, dataloader):
        ...
        return scores, masks, labels_gt, masks_gt, visualize
    ...
    def _predict(self, images):
        ...
        with torch.no_grad():
            ...
            #visualize.shape=[b,28,28,3,8,8]
            visualize = visualize.reshape(batchsize, 28, 28, 3, 8, 8)
            #visualize.shape=[b,3,28,8,28,8]
            visualize = visualize.transpose(0, 3, 1, 4, 2, 5)
            #visualize.shape=[b,3,224,224]
            visualize = visualize.reshape(batchsize, 3, 224, 224)

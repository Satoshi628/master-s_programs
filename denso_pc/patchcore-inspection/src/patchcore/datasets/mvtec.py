...
class MVTecDataset(torch.utils.data.Dataset):
    ...
    def __init__(
        ...
        self.data_to_iterate_temp = self.data_to_iterate
    ...
    def data_split(self, data_split):
        if self.split == DatasetSplit.TRAIN:
            #データ数が0にならないように調整
            length = len(self.data_to_iterate_temp)
            min_idx = int(length*data_split[0])
            max_idx = max(1, int(length*data_split[1]))
            self.data_to_iterate = self.data_to_iterate_temp[min_idx:max_idx]
...
class MVTecDatasetFew(torch.utils.data.Dataset):
    """
    PyTorch Dataset for MVTec.
    """
    ...
    def get_image_data(self):
        ...
        for classname in self.classnames_to_use:
            ...
            for anomaly in anomaly_types:
                ...
                if self.split == DatasetSplit.TRAIN:
                        n_images = len(imgpaths_per_class[classname][anomaly])
                        if self.data_rate <= 1.0:
                            choice_num = int(n_images * self.data_rate)
                            choice_idx = np.random.choice(list(range(n_images)), size=choice_num, replace=False).tolist()
                        else:
                            choice_idx = np.random.choice(list(range(n_images)), size=int(self.data_rate), replace=False).tolist()

                        imgpaths_per_class[classname][anomaly] = [imgpaths_per_class[classname][anomaly][idx] for idx in choice_idx]
...
if __name__ == "__main__":#データセットのチェック

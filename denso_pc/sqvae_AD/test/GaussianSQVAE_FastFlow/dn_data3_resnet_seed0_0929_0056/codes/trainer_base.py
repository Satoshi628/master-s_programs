...
from model import GaussianSQVAE, VmfSQVAE, GaussianSQVAE_FastFlow
from util import *

MODEL_DICT = {"GaussianSQVAE": GaussianSQVAE,
                "VmfSQVAE":VmfSQVAE,
                "GaussianSQVAE_FastFlow":GaussianSQVAE_FastFlow}
class TrainerBase(nn.Module):
    def __init__(self, cfgs, flgs, train_loader, val_loader, test_loader):
        ...
        self.model = nn.DataParallel(MODEL_DICT[cfgs.model.name](cfgs, flgs))
        # self.model = eval(
    ...
    def _make_path(self):
        ...
        self.path = os.path.join(self.cfgs.path, "{}_{}_seed{}_{}".format(
            self.cfgs.category, self.cfgs.network.name, self.cfgs.train.seed,timestamp))
        print(self.path)
        if self.flgs.save:

...
class GaussianVectorQuantizer(VectorQuantizer):
    ...
    def _quantize(self, z_from_encoder, var_q, flg_train=True, flg_quant_det=True):
        ...
        logit = -self._calc_distance_bw_enc_codes(z_from_encoder_permuted, self.codebook.detach(), 0.5 * precision_q)
        ...
        if self.training:
            ...
            # [bHW, 512]
            # labels[bHW] 0or 1
            class0, class1 = encodings.chunk(2, dim=-1)
            #class0[labels == 0] [N, 256]
            #class0[labels == 0].sum(dim=-1) [N]
            class0_loss = (1 - class0[labels == 0].sum(dim=-1))**2
            class1_loss = (1 - class1[labels == 1].sum(dim=-1))**2
            loss = class0_loss.mean() + class1_loss.mean()
        ...
        kld_discrete = torch.mean(probabilities * log_probabilities)
        kld_continuous = self._calc_distance_bw_enc_dec(z_from_encoder, z_to_decoder.detach(), 0.5 * precision_q).mean()
        loss = kld_discrete + kld_continuous
        z_to_decoder = z_from_encoder + (z_to_decoder - z_from_encoder).detach()

        return z_to_decoder, loss, indices

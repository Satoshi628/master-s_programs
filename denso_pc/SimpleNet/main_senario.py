...
_DATASETS = {
    "mvtec": ["datasets.mvtec", "MVTecDataset_Scenario"],
}
...
def run(
    ...
    for dataloader_count, dataloaders in enumerate(list_of_dataloaders):
        ...
        for i, SimpleNet in enumerate(simplenet_list):
            ...
            if not test:
                i_auroc, p_auroc, pro_auroc, OnlyL1Score, OtherL1Score, results = SimpleNet.train(dataloaders["training"], dataloaders["testing"])
                if len(results) != 0:
                    with open(os.path.join(SimpleNet.ckpt_dir, "training.txt"), "w") as f:
                        f.write("\t".join(list(results[0].keys())) + "\n")
                        for res in results:
                            f.write("\t".join([f"{v}" for v in res.values()]) + "\n")

            else:
                i_auroc, p_auroc, pro_auroc, OnlyL1Score, OtherL1Score =  SimpleNet.test(dataloaders["training"], dataloaders["testing"])
...
@click.option("--best_type", type=str, default="score")
def net(
        backbone_names,
    ...
    def get_simplenet(input_shape, device):
        ...
        for backbone_name, layers_to_extract_from in zip(
            ...
            simplenet_inst = simplenet.SimpleNet(best_type, device)
            simplenet_inst.load(
                backbone=backbone,

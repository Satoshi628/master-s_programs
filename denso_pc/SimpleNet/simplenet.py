...
from datasets.mvtec import _SCENARIO
import common
import metrics
...
def see_img_heatmap(data, segresult, save_path, image_name):  #C,H,W
    data = data.cpu()
    ...
    y2max = 255
    y2min = 0
    segresult = np.round((y2max - y2min) * segresult + y2min)
    ...
    overlay = data.copy()
    data = cv2.addWeighted(heatmap, alpha2, overlay, 1 - alpha2, 0, overlay)
    
    if not os.path.isdir(os.path.join(save_path, "images")):
        os.mkdir(os.path.join(save_path, "images"))
    image_name = os.path.split(image_name)[-1]
    cv2.imwrite(os.path.join(save_path, "images", image_name), data)
...
class SimpleNet(torch.nn.Module):
    def __init__(self, best_type, device):
    ...
    def _embed(self, images, detach=True, provide_patch_shapes=False, evaluation=False):
        ...
        features = [features[layer][0] if isinstance(features[layer], tuple) else features[layer] for layer in self.layers_to_extract_from]
    ...
    def test(self, training_data, test_data):
        ...
        scores, segmentations, features, labels_gt, masks_gt, img_paths, images = self.predict(test_data)
        aggregator["scores"].append(scores)
        aggregator["segmentations"].append(segmentations)
        aggregator["features"].append(features)

        anomaly = [x[1] for x in test_data.dataset.data_to_iterate]
        
        if test_data.dataset.scenario == "A2N":
            anomaly_flag = {test_data.dataset.broken_type:[test_data.dataset.broken_type in ano for ano in anomaly]}
        elif test_data.dataset.scenario == "N2A":
            anomaly_flag = {"pseudo_anomaly":["pseudo_anomaly" in ano for ano in anomaly]}
        elif test_data.dataset.scenario == "Normal" and "good-pseudo_anomaly" in anomaly: # N2A Normal
            anomaly_flag = {"pseudo_anomaly":["pseudo_anomaly" in ano for ano in anomaly]}
        else: #A2N Normal
            anomaly_flag = {broken_type:[broken_type in ano for ano in anomaly] for broken_type in _SCENARIO[test_data.dataset.classnames_to_use]}
        ...
        sort_idx = np.argsort(img_paths)
        img_paths = [img_paths[idx] for idx in sort_idx]
        scores = scores[sort_idx]
        labels_gt = labels_gt[sort_idx]
        res_score = labels_gt - scores

        result_path = os.path.join(self.ckpt_dir, "_test.txt")
        with open(result_path, "w") as f:
            for path, score, label, res in zip(img_paths, scores, labels_gt, res_score):
        ...
        for img, seg, path in zip(images, segmentations, img_paths):
            see_img_heatmap(img, seg, self.ckpt_dir, path)
        
        anomaly_labels = [
            int(not "good" in x[1]) for x in test_data.dataset.data_to_iterate
        ]
        ...
        for broken_type, flag in anomaly_flag.items():
            ...
            OnlyL1Score[f"{broken_type}"] = metrics.compute_AUAnomaly_Curve(scores[flag])
            OtherL1Score[f"{broken_type}"] = metrics.compute_imagewise_retrieval_metrics(scores[~flag], anomaly_labels[~flag])["auroc"]
        ...
        return auroc, full_pixel_auroc, pro, OnlyL1Score, OtherL1Score
    ...
    def _evaluate(self, test_data, scores, segmentations, features, labels_gt, masks_gt):
        ...
        if test_data.dataset.scenario == "A2N":
            ...
            anomaly_flag = {"pseudo_anomaly":["pseudo_anomaly" in ano for ano in anomaly]}
        elif test_data.dataset.scenario == "Normal" and "pseudo_anomaly" in anomaly: # N2A Normal
            anomaly_flag = {"pseudo_anomaly":["pseudo_anomaly" in ano for ano in anomaly]}
        else: #A2N Normal
            anomaly_flag = {broken_type:[broken_type in ano for ano in anomaly] for broken_type in _SCENARIO[test_data.dataset.classnames_to_use]}
        ...
        for broken_type, flag in anomaly_flag.items():
            flag = np.array(flag)
            anomaly_labels = np.array(labels_gt)
            OnlyL1Score[f"{broken_type}"] = metrics.compute_AUAnomaly_Curve(scores[flag])
            OtherL1Score[f"{broken_type}"] = metrics.compute_imagewise_retrieval_metrics(scores[~flag], anomaly_labels[~flag])["auroc"]
    ...
    def train(self, training_data, test_data):
        ...
        best_score_ckpt_path = os.path.join(self.ckpt_dir, "best_score_ckpt.pth")
        best_loss_ckpt_path = os.path.join(self.ckpt_dir, "best_loss_ckpt.pth")

        if self.best_type == "score":
            ckpt_path = best_score_ckpt_path
        else:
            ckpt_path = best_loss_ckpt_path
        ...
        if os.path.exists(ckpt_path):
            ...
            scores, segmentations, features, labels_gt, masks_gt, img_paths, images = self.predict(test_data)
            auroc, full_pixel_auroc, anomaly_pixel_auroc, OnlyL1Score, OtherL1Score = self._evaluate(test_data, scores, segmentations, features, labels_gt, masks_gt)
            
            return auroc, full_pixel_auroc, anomaly_pixel_auroc, OnlyL1Score, OtherL1Score, []
        ...
        for i_mepoch in range(self.meta_epochs):
            ...
            loss, p_true, p_fake = self._train_discriminator(training_data)

            # torch.cuda.empty_cache()
            
            scores, segmentations, features, labels_gt, masks_gt, img_paths, images = self.predict(test_data)
            auroc, full_pixel_auroc, pro, OnlyL1Score, OtherL1Score = self._evaluate(test_data, scores, segmentations, features, labels_gt, masks_gt)
            ...
            if best_record is None:
                best_record = [auroc, full_pixel_auroc, pro, OnlyL1Score, OtherL1Score]
                update_state_dict(best_score_state_dict)
                ...
                if auroc > best_record[0]:
                    ...
                    best_record[2] = pro
                    best_record[3] = OnlyL1Score
                    best_record[4] = OtherL1Score
                    update_state_dict(best_score_state_dict)
                    # state_dict = OrderedDict({k:v.detach().cpu() for k, v in self.state_dict().items()})

            if best_loss is None:
                best_loss = loss
                update_state_dict(best_loss_state_dict)
                # state_dict = OrderedDict({k:v.detach().cpu() for k, v in self.state_dict().items()})
            else:
                if loss < best_loss:
                    best_loss = loss
            ...
            result_list.append({"epoch":i_mepoch, "loss":loss, "p_true":p_true, "p_fake":p_fake, "auroc":auroc, "full_pixel_auroc":auroc, "pro":pro})
        torch.save(best_score_state_dict, best_score_ckpt_path)
        torch.save(best_loss_state_dict, best_loss_ckpt_path)
        
        return *best_record, result_list
    ...
    def _train_discriminator(self, input_data):
        ...
        with tqdm.tqdm(total=self.gan_epochs) as pbar:
            for i_epoch in range(self.gan_epochs):
                ...
                pbar.update(1)
        return all_loss, all_p_true, all_p_fake
    ...
    def _predict_dataloader(self, dataloader, prefix):
        ...
        return scores, masks, features, labels_gt, masks_gt, img_paths, images

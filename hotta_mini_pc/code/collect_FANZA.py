...
def video_download(file_name):
    ...
    option_relative_position = [80,95]
    file_bar_position = [320,450]
    pag.click(*video_position, button="right")
    pag.move(*option_relative_position)
    pag.click()
    sleep(1)
    pag.click(*file_bar_position)
    pag.hotkey("ctrl", "a")
    pag.press("backspace")
    pag.write(f"C:\\Users\\hotta_mini\\affi_data\\data\\{file_name}.mp4")
    pag.press("enter")
    sleep(10)
"""
def video_download():
...
def get_ad_twitter_link(driver):
    ...
    sleep(1)
    #"https://al.dmm.co.jp/?lurl=https%3A%2F%2Fwww.dmm.co.jp%2Fdigital%2Fvideoa%2F-%2Fdetail%2F%3D%2Fcid%3Dssis00853%2F&af_id=doshikori721-001&ch=toolbar&ch_id=twitter "
...
def get_sample(driver, df):
    
    ranking_Xpath = "//*[@id=\"main-ds\"]/div[2]/table[{}]/tbody/tr/td[{}]/div/p/a"
    ...
    for idx in range(40):
        data = dict()
        #作品一つクリック
        xpath_click(driver, ranking_Xpath.format(idx//2+1,idx%2 + 1))
        ...
        try:
            df.to_csv("C:\\Users\\hotta_mini\\affi_data\\data.csv", encoding="shift-jis", index=False)
        except Exception as e:
            print(f"ERROR:save csv")
            print(e)
            df.to_csv("C:\\Users\\hotta_mini\\affi_data\\data.csv", encoding="shift-jis", errors='ignore', index=False)

if  os.path.isfile("C:\\Users\\hotta_mini\\affi_data\\data.csv"):
    df = pd.read_csv("C:\\Users\\hotta_mini\\affi_data\\data.csv", encoding="shift-jis")
else:
    df = None
...
#FANZA動画ランキングに移動
driver.get('https://www.dmm.co.jp/digital/videoa/-/ranking/=/term=realtime/')

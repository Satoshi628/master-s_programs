...
def get_args():
    parser = argparse.ArgumentParser(description="FANZAデータ収集プログラム(すべて)")
    parser.add_argument(
        "-d", "--data_folder", default="", help="save folder")
...
def video_download(video_path):
    ...
    save_path = os.path.join(video_path, f"{file_name}.mp4")
...
def get_sample(driver, df, csv_path, video_path):
    ...
    for prod_e in product_elems:
        ...
        #ダウンロード
        if sample_flag:
            video_flag, video_url, save_path  = video_download(video_path)
...
args = get_args()
save_dir = os.path.join("C:\\Users\\hotta_mini\\affi_data", args.data_folder)
if not os.path.exists(save_dir):
    os.mkdir(save_dir)

csv_path = f"C:\\Users\\hotta_mini\\affi_data\\data_all.csv"
...
#FANZA動画ランキングに移動
url = "https://www.dmm.co.jp/digital/videoa/-/list/=/sort=ranking/page={}/"
...
for i in range(2, 10):
    get_sample(driver, df, csv_path, save_dir)
